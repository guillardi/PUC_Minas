...
# output <- matrix(, nrow = n_distinct(mpi$responsavelCadastro, year(mpi$dataMovimentacao)), ncol = 4,
#                  dimnames = list(c(1:21), c("responsavel", "ano", "mpis","bens")))



mpiSumResponsavelCadastro <- mpi %>% group_by(responsavelCadastro, ano = year(dataMovimentacao), id) %>%
  tally() %>% tally(name = "MPIs") %>% ungroup() %>%
  mutate(bens = (group_by(mpi, responsavelCadastro, ano = year(dataMovimentacao)) %>%
                   tally())[[3]],
         mediaMPIs = as.double(format(MPIs / sum(mpiSumResponsavelCadastro$MPIs) * 100, digits=3, nsmall=3)),
         mediaBens = as.double(format(bens / sum(mpiSumResponsavelCadastro$bens) * 100, digits=3, nsmall=3)))

sum(mpiSumResponsavelCadastro$MPIs)
sum(mpiSumResponsavelCadastro$bens)
sum(mpiSumResponsavelCadastro$mediaMPIs)
sum(mpiSumResponsavelCadastro$mediaBens)
...
mpiSumResponsavelBens <- mpi %>% filter(year(dataMovimentacao) == 2019, 
                                        month(dataMovimentacao) == 10,
                                        (responsavelCadastro == "Marcio Guillardi da Silva" | responsavelCadastro == "SCMC"),
                                        nivelSuperior %in% c("CNC, 01º ANDAR", "CNC, 02º ANDAR", "CNC, 4º SUBSOLO", "CNC, 08º ANDAR", "CNC, 15º ANDAR", "CNC, 18º ANDAR")) %>% 
  group_by(responsavel, cedente, sala, tombamento) %>%
  tally() %>% tally(name = "MPIs")

write_excel_csv2(mpiSumResponsavelBens, ".\\data\\movimentacoesPGTResponsavel.csv", na = "NA", append = FALSE, delim = ";", quote_escape = "double")

#

mpi_teste <- mpi %>% filter(sala == "SL. Nº 203")
mpi_teste <- mpi %>% filter(id == "73887")

#
# (group_by(mpi, responsavelCadastro, ano = year(dataMovimentacao)) %>%
#     tally())[[3]]
# 
# output[,"responsavel"]  <-  c_responsavel$responsavelCadastro
# output[,"ano"]          <-  c_responsavel$ano
# output[,"mpis"]         <-  c_responsavel$n
# 
# c_responsavel <- mpi %>% group_by(responsavelCadastro, ano = year(dataMovimentacao)) %>%
#   tally()
# 
# output[,"bens"]         <-  c_responsavel$n
# 
# 
...
#
# group_by responsavel + ano + id => tally + tally = total de MPIs cadastradas
# 
# group_by responsavel + ano      => tally         = total de MPIs cadastradas
#
for (t in 1:21) {
  print(t)
 # output[[t,'responsavel']] <-  mpi$responsavelCadastro[[t]]
}
...
scatter3d(x = mpi_ST$mes, y = mpi_ST$`Ano 2018`, z = mpi_ST$`Ano 2019`, point.col = "blue")
